...
import { 
  MapPin, Star, Wifi, Wind, Coffee, Droplets, Car, Dumbbell, 
  Utensils, Check, ChevronLeft, Calendar, Users, ShieldCheck, 
  Heart, Share2, Camera, BedDouble, Maximize, Bath, CheckCircle2
} from 'lucide-react';
...
const HotelDetailPage: React.FC<HotelDetailPageProps> = ({ hotelId, onBack }) => {
  ...
  const hotelData = {
    ...
    rating: 4.9,
    reviews: 328,
    description: "ศรีพันวา ภูเก็ต ลักชัวรี พูลวิลล่า รีสอร์ทหรูสไตล์ทรอปิคอลร่วมสมัย ตั้งอยู่บนปลายแหลมพันวาที่ยื่นออกไปในทะเลอันดามัน รายล้อมด้วยวิวทะเลแบบพาโนรามา 300 องศา ให้ความเป็นส่วนตัวสูงสุด พร้อมสระว่ายน้ำส่วนตัวในทุกวิลล่า",
    images: [
    ...
    rooms: [
      {
        ...
        features: ["สระว่ายน้ำส่วนตัว", "อ่างอาบน้ำ", "ระเบียงส่วนตัว", "ฟรีมินิบาร์"]
      },
      {
        id: 2,
        ...
        features: ["สระว่ายน้ำอินฟินิตี้", "ห้องนั่งเล่นแยก", "Sound System", "บริการบัตเลอร์"]
      },
      {
        id: 3,
        ...
        features: ["ชั้นบนสุด", "ครัวขนาดเล็ก", "จากุซซี่", "วิวพระอาทิตย์ตก"]
      }
    ]
  };

  const selectedRoom = hotelData.rooms.find(r => r.id === selectedRoomId) || hotelData.rooms[0];
      ...
      <div className="container mx-auto px-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
                        ...
                        <div className="flex items-center gap-1">
                            <span className="font-bold text-gray-900">{hotelData.rating}</span>
                            <span>ยอดเยี่ยม ({hotelData.reviews} รีวิว)</span>
                        </div>
                    </div>
                </div>
                <div className="flex gap-2">
                     <button className="p-2.5 bg-white rounded-full shadow-md text-gray-700 hover:text-red-500 transition-colors border border-gray-100">
                        <Heart size={20} />
                     </button>
                     <button className="p-2.5 bg-white rounded-full shadow-md text-gray-700 hover:text-brand-500 transition-colors border border-gray-100">
                        <Share2 size={20} />
                     </button>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 h-[300px] md:h-[450px] rounded-3xl overflow-hidden mb-10 relative shadow-sm border border-gray-100">
            <div className="md:col-span-2 h-full">
                <img src={hotelData.images[0]} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700 cursor-pointer" alt="Main" />
            </div>
            <div className="md:col-span-1 flex flex-col gap-2 h-full">
                <div className="h-1/2 overflow-hidden">
                    <img src={hotelData.images[1]} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700 cursor-pointer" alt="Sub 1" />
                </div>
                <div className="h-1/2 overflow-hidden">
                    <img src={hotelData.images[2]} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700 cursor-pointer" alt="Sub 2" />
                </div>
            </div>
            <div className="md:col-span-1 h-full overflow-hidden relative group cursor-pointer">
                <img src={hotelData.images[3]} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="Sub 3" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-10">
                ...
                <section className="bg-white rounded-3xl p-6 md:p-8 border border-gray-100 shadow-sm">
                    <h3 className="text-xl font-bold text-gray-900 mb-4">รายละเอียดที่พัก</h3>
                    <p className="text-gray-600 leading-relaxed mb-8 font-light">
                        {hotelData.description}
                    </p>

                    <h4 className="font-bold text-gray-900 mb-4">สิ่งอำนวยความสะดวก</h4>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {hotelData.facilities.map((fac, i) => (
                            <div key={i} className="flex items-center gap-3 text-sm text-gray-600">
                                <div className="w-8 h-8 rounded-full bg-brand-50 text-brand-500 flex items-center justify-center flex-shrink-0">
                                    {fac.icon}
                                </div>
                                {fac.name}
                            </div>
                ...
                <section>
                    <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                        <BedDouble className="text-brand-500" /> เลือกห้องพัก
                    </h3>
                            ...
                            >
                                ...
                                <div className="p-6 flex-1 flex flex-col">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        {selectedRoomId === room.id && (
                                            <span className="bg-brand-50 text-brand-600 text-[10px] font-bold px-2 py-1 rounded-full border border-brand-100 flex items-center gap-1">
                                    ...
                                    <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500 mb-4">
                                        <div className="flex items-center gap-1">
                                            <Maximize size={14} className="text-brand-400" /> {room.size}
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <BedDouble size={14} className="text-brand-400" /> {room.bed}
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <Wind size={14} className="text-brand-400" /> {room.view}
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-auto flex items-center justify-between pt-4 border-t border-gray-50">
                                        <div>
                                            <div className="flex items-baseline gap-1">
                                                <span className="text-xl font-bold text-brand-600">฿{room.price.toLocaleString()}</span>
            ...
            <div className="lg:col-span-1">
                 <div className="sticky top-28 bg-white rounded-3xl shadow-xl border border-gray-100 p-6 overflow-hidden">
                    ...
                    <div className="mb-6 pb-6 border-b border-gray-100">
                        <p className="text-sm font-bold text-gray-900 mb-1">{selectedRoom.name}</p>
                        <div className="flex items-baseline gap-2">
                             <span className="text-3xl font-extrabold text-brand-600">฿{totalPrice.toLocaleString()}</span>
                             <span className="text-sm text-gray-500">/ {nights} คืน</span>
                        </div>
                        <p className="text-xs text-gray-400 mt-1">รวมภาษีและค่าธรรมเนียมแล้ว</p>
                    </div>

                    <div className="space-y-4 mb-6">
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1.5 ml-1">เช็คอิน</label>
                                <div className="relative">
                                    <input type="date" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-xs font-bold focus:ring-2 focus:ring-brand-200 outline-none" />
                                </div>
                            </div>
                             <div>
                                <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1.5 ml-1">เช็คเอาท์</label>
                                <div className="relative">
                                    <input type="date" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-xs font-bold focus:ring-2 focus:ring-brand-200 outline-none" />
                                </div>
                            </div>
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1.5 ml-1">ระยะเวลาเข้าพัก</label>
                            <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-xl p-3">
                                ...
                                <div className="flex items-center gap-3">
                                    <button 
                                        ...
                                        className="w-7 h-7 rounded-full bg-white border border-gray-200 flex items-center justify-center hover:border-brand-300 text-brand-600 disabled:opacity-50"
                                        disabled={nights <= 1}
                                    >-</button>
                                    <span className="font-bold text-gray-900 w-4 text-center">{nights}</span>
                                    <button 
                                        onClick={() => setNights(nights + 1)}
                                        className="w-7 h-7 rounded-full bg-white border border-gray-200 flex items-center justify-center hover:border-brand-300 text-brand-600"
                                    >+</button>
                                </div>
                            </div>
                        </div>
                    ...
                    <Button fullWidth className="!py-3.5 shadow-lg shadow-brand-500/30 mb-3">
                        จองที่พัก
                    </Button>
